...
def custom_map_per_video(df_groundtruth, df_predictions, dict_vids):

  for category, list_vids in dict_vids.items():
    for video in list_vids:
      df_custom_gt, df_custom_predictions = create_pd_custom_choice(df_groundtruth, df_predictions, [video])
      df_custom = merge_groundtruth_and_predictions(df_custom_gt, df_custom_predictions)
      precision, recall = calculate_precision_recall(df_custom)
      print("{},{},{}".format(video, category, compute_average_precision(precision, recall)))
...
def run_evaluation(groundtruth, predictions):
  ...
  # Custom map per video
  # dict_vids = create_dict_vids()
  # custom_map_per_video(df_groundtruth, df_predictions, dict_vids)
